...
function Help() {
	...
	// and the field on the help table which links to volunteer and project plus
	...
	const volunteerRecordset = useRecords(volunteerTable && volunteerNameField ? volunteerTable.selectRecords({sorts: [
        // sort by 'My field' in ascending order...
        {field: volunteerNameField.id}
     ]}) : null);
		...
		return(
			<div>
				<FormField label="Project name">
					<Input value={projectName} onChange={e => projectNameChange(setProjectName, setProjectRecId, e.target.value, setChosenProject)} />
				</FormField>
				
				<br></br>
				<Text size="large">{chosenProject}</Text>
				<br></br>
				...
				{projectRecords.map(record => (
					<li key={record.id}>
						<TextButton
							variant="dark"
							size="xlarge"
							onClick={() => {chooseProject(setProjectRecId, record.id, projectRecords, projectNameField, setChosenProject)}}
...
function chooseProject(setProjectRecId, projectRecordId, projectRecords, projectNameField, setChosenProject){
	...
	setProjectRecId(projectRecordId);
	//get the project Name
	
	for (let rec of projectRecords){
		if (rec.id == projectRecordId) {
			let vname = rec.getCellValue(projectNameField.id);
			//console.log(vname);
			setChosenProject(vname);
		}
	}

}

function projectNameChange(settera, setterb, setterval,setChosenProject){
	settera(setterval);
	setterb("");
	setChosenProject("");
}
...
function SettingsMenu(props) {
    ...
    const getLinkedVolunteerTable = () => {
		...
		//console.log("finsihed getLinkedVolunteerTable");
   };
